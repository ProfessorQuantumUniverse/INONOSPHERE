...
def process_rinex_advanced(obs_file, nav_files):
    ...
    for sv in tqdm(valid_svs, desc="🛰️ Verarbeite GPS & Galileo"):
        ...
        if not p1_var or not p2_var: continue
...
def plot_professional_ionosphere_map(lats, lons, vtec, lat_sta, lon_sta):
    ...
    # Raster für die Karte definieren
    grid_lon, grid_lat = np.mgrid[lon_sta-10:lon_sta+10:300j, lat_sta-8:lat_sta+8:300j]
    
    # 1. RBF Interpolation (Organischere Übergänge als linear/Delaunay)
    # epsilon steuert die Weichheit der Kanten.
    rbf = Rbf(lons, lats, vtec, function='linear', epsilon=2.0)
    grid_vtec = rbf(grid_lon, grid_lat)

    # 2. Masking mit KDTree (Schneidet ab, was zu weit von echten Messungen weg ist)
    # Verhindert, dass das RBF-Modell die ganze Welt einfärbt.
    tree = cKDTree(np.c_[lons, lats])
    # Finde für jeden Pixel im Grid den Abstand zum nächsten Messpunkt (IPP)
    dist, _ = tree.query(np.c_[grid_lon.ravel(), grid_lat.ravel()])
    dist = dist.reshape(grid_lon.shape)
    
    # Alles, was weiter als ~2.5 Grad (ca. 250km) von einem echten Punkt weg ist, wird ausgeblendet
    MAX_DISTANCE_DEG = 2.5 
    grid_vtec[dist > MAX_DISTANCE_DEG] = np.nan

    # Gauss-Filter für den allerletzten feinen Schliff
    ...
    # 3. Gridlines (Längen-/Breitengrade) für einen wissenschaftlichen Look
    gl = ax.gridlines(draw_labels=True, linewidth=0.5, color='gray', alpha=0.5, linestyle=':')
    gl.top_labels = False
    gl.right_labels = False
    
    levels = np.linspace(0, max(20, np.nanmax(grid_vtec_smooth)), 40)
    contour = ax.contourf(grid_lon, grid_lat, grid_vtec_smooth, levels=levels, cmap='plasma', alpha=0.85, transform=ccrs.PlateCarree())
    
    # Tracks etwas edler zeichnen (weiß umrandet)
    ax.scatter(lons, lats, color='white', s=8, transform=ccrs.PlateCarree(), alpha=0.8)
    ax.scatter(lons, lats, color='black', s=2, transform=ccrs.PlateCarree(), alpha=1.0, label='IPP Messpunkte')
    ...
    cbar.set_label('VTEC (Vertical TEC)[TECU]', fontsize=13, weight='bold')
    cbar.ax.tick_params(labelsize=11)
    
    plt.title(f'Lokale VTEC Ionosphären-Map\nStation: {lon_sta:.2f}E, {lat_sta:.2f}N | Höhe: 350km', pad=15, fontsize=15, weight='bold')
...
if __name__ == '__main__':
    ...
    # NEU: Du kannst jetzt eine LISTE von Nav-Dateien übergeben!
    # z.B. die GPS Datei (.26n) und die Galileo Datei (.26l)
    ...
    lats, lons, vtecs, sta_lat, sta_lon = process_rinex_advanced(RINEX_OBS_FILE, RINEX_NAV_FILES)
